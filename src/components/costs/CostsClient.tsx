...
export default function CostsClient({
  ...
  const [activeTab, setActiveTab] = useState<"direct" | "consumables" | "monthly">("direct");
  ...
  return (
    <main className="max-w-screen-2xl mx-auto px-6 py-8">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-extrabold tracking-tight text-on-surface">Cost Accounting</h1>
      ...
      {/* Tab Content */}
      {activeTab === "direct" && <DirectTab order={order} />}
      {activeTab === "consumables" && (
        <ConsumablesTab costs={costs.filter((c) => c.costType === "CONSUMABLES")} settings={settings} />
      )}
      {activeTab === "monthly" && (
        <MonthlyFixedTab costs={costs.filter((c) => c.costType === "MONTHLY_FIXED")} />
      )}
    </main>
  );
}

function DirectTab({ order }: { order: Order }) {
              ...
              {parsedItems.map((item, idx) => (
                <div key={idx} className="p-4 rounded-xl bg-surface-container-low border-l-4 border-primary">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="font-headline font-bold text-lg">¥{item.amount.toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
...
function MonthlyFixedTab({ costs }: { costs: Cost[] }) {
  const router = useRouter();
